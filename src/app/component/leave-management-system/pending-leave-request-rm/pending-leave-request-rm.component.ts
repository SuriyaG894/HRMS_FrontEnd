...
@Component({
  selector: 'app-pending-leave-request-rm',
  imports: [CommonModule,FormsModule],
  templateUrl: './pending-leave-request-rm.component.html',
  styleUrl: './pending-leave-request-rm.component.css'
})
export class PendingLeaveRequestRmComponent {
...
resetFilters() {
  ...
  this.getMockLeaves();
    ...
    getMockLeaves() {
    this.leaveService.getPendingLeaveRequestForRM().subscribe({
  ...
  this.pagedData = dataSource.slice(start, end);
    ...
    this.leaveService.cancelRejectedLeaveByRM(leaveId,approveRequestObj).subscribe({
      ...
      error:(error)=>{
        console.log(error);
        Swal.fire('Leave Status', error, 'error');
      }
    });
    }
